...
app.controller('myCtrl', function ($scope,$http,$location,$route,$routeParams) {
	...
	$scope.newsNavs = function(){
		...
		if($scope.news[prv]!==undefined){
			navz += '<a href="#/news/'+prv+'" class="btn btn-xs btn-danger pull-left"><i class="fa fa-arrow-left"></i> Previous News</a>';
		}
		if($scope.news[nxt]!==undefined){
			navz += '<a href="#/news/'+nxt+'" class="btn btn-xs btn-danger pull-right"><i class="fa fa-arrow-right"></i> Next News</a>';
		}
		navz +='<div class="clearfix"></div>';
		return navz;
	}
	...
	$http({
		...
		about = JSON.parse(response.data);
		dbSave("about", about);
